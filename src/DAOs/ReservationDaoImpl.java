...
public class ReservationDaoImpl implements ReservationDao {
    ...
    @Override
    public List<Reservation> getAllCustomerFutureReservations(int customer_id) {
        try {
            Statement stmt = db.connect();
            assert stmt != null;
            ZoneId italyZone = ZoneId.of("Europe/Rome");
            // Crea una data nel fuso orario italiano
            LocalDate today = LocalDate.now(italyZone);
            ResultSet rs = stmt.executeQuery("select reservation.id, reservation.date, court, start_hour, finish_hour, reservation.price, case when num_of_rents is null then '0' else num_of_rents end as num_of_rentingkit from (reservation join time_slots on reservation.time_slot = time_slots.id) left join rentingkit_reservation rr on rr.reservation = reservation.id where customer = '" + customer_id + "' and date > '" + today + "'");
            List<Reservation> reservations = makeReservationsList(rs);
            rs.close();
            return reservations;
        } catch (SQLException e) {
    ...
    @Override
    public ArrayList<Integer> getReservationsId(int customer) {
        Statement stmt = db.connect();
        assert stmt != null;
        try {
            ResultSet rs = stmt.executeQuery("select reservation.id from reservation where customer = '" + customer + "'");
    ...
    @Override
    public boolean checkTestReservation(Customer customer, Date date) {
        try {
            Statement stmt = db.connect();
            assert stmt != null;
            ResultSet rs = stmt.executeQuery("select * from reservation where customer = '" + customer.getId() + "' and date = '" + date + "'");
            if (!rs.next()) {
                rs.close();
                db.disconnect();
                return false;
    ...
    public boolean insertReservation(Reservation reservation, boolean updatePoints, boolean updateWallet) {
        try {
            ...
            assert stmt != null;
            stmt.executeUpdate("INSERT INTO reservation (date, court, customer, time_slot, price, isPremium) VALUES ('" + reservation.getDate() + "', '" + reservation.getCourt().getId() + "', '" + reservation.getCustomer().getId() + "', '" + reservation.getTime_slot().getId() + "', '" + reservation.getPrice() + "', '" + reservation.getIsPremium() + "')");
            ResultSet rs = stmt.executeQuery("select id from reservation where date = '" + reservation.getDate() + "' and court = '" + reservation.getCourt().getId() + "' and customer = '" + reservation.getCustomer().getId() + "' and time_slot = '" + reservation.getTime_slot().getId() + "'");
            rs.next();
